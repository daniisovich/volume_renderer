	...
	namespace glfw {
		...
		void Controller::FramebufferSizeCallback(GLFWwindow* window, int width, int height) {
			...
			if (m_renderer) {
				...
				gl::Framebuffer resized_framebuffer{ GL_RGBA8, {uint32_t(width), uint32_t(height)}, 0, 0 };
		...
		void Controller::KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods) {
			...
			if (key == GLFW_KEY_UP && (action == GLFW_PRESS || action == GLFW_REPEAT)) {
				addSmoothStepEnd(float(m_speed) * m_smooth_step_factor);
			}
			if (key == GLFW_KEY_DOWN && (action == GLFW_PRESS || action == GLFW_REPEAT)) {
				addSmoothStepEnd(-float(m_speed) * m_smooth_step_factor);
			}
			if (key == GLFW_KEY_RIGHT && (action == GLFW_PRESS || action == GLFW_REPEAT)) {
				addSmoothStepStart(float(m_speed) * m_smooth_step_factor);
			}
